...
const createSchema = () => {
  ...
  props.inputs.forEach((input) => {
    ...
    const isRequired =
      props.stepInfo.stepType === "claim_seat" || input.isRequired;
...
watch(isSubmitting, (errors, oldErrors) => {
  console.log(errors)
})

// Make fields behave nicely
// Note: defineField is tricky with dynamic fields in loop, so we stick to setFieldValue/values for now
// but we ensure validation triggers on blur/submit by configuration above.
...
// Ensure state is preserved when unmounting (e.g. hitting Back)
onBeforeUnmount(() => {
  emit("save", values, props.stepInfo.id);
});
</script>
...
<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div class="space-y-2">
      ...
      <p v-if="stepInfo.description" class="text-sm text-zinc-500">
        <span v-html="stepInfo.description"></span>
      </p>
    </div>
    ...
    <form @submit.prevent="onSubmit" class="space-y-6">
      <div v-for="input in inputs" :key="input.id" class="space-y-2">
        <Label
          >{{ input.label }}
          <span
            v-if="stepInfo.stepType === 'claim_seat' || input.isRequired"
            class="text-red-500"
            >*</span
          ></Label
        >

        <p v-if="input.description" class="text-xs text-zinc-400 mb-1">
          <span v-html="input.description"></span>
        </p>

        <!-- Text / Email / Phone / Number -->
        <Input
          v-if="
            ['text', 'email', 'phone', 'number', 'select'].includes(
              input.inputType,
        ...
        >
          ...
          <SelectContent>
            <SelectItem
              v-for="opt in getOptions(input.options)"
              :key="opt.value || opt"
              :value="opt.value || opt"
            >
              {{ opt.label || opt }}
        ...
        <!-- File -->
        <div
          v-else-if="input.inputType === 'file'"
        ...
        >
          <Input
            ...
            @change="
              (e: any) => {
                setFieldValue(`input_${input.id}`, e.target.files[0]);
                // Trigger validation manually if needed, or rely on onSubmit
              }
            "
        ...
        <p
          v-if="(errors as any)[`input_${input.id}`]"
